...
static char **fill_envp(struct dhcpMessage *packet)
{
	...
	if (packet) {
		for (i = 0; dhcp_options[i].code; i++) {
			if (get_option(packet, dhcp_options[i].code)) {
				...
				if (dhcp_options[i].code == DHCP_VENDOR_SPEC)
					num_options++; /* for unifi-addr */
			}
		}
		if (packet->siaddr)
			num_options++;
	...
	while (*opt_name) {
		...
		} else if (dhcp_options[i].code == DHCP_VENDOR_SPEC) {
			if (temp[0] == 1 && temp[1] == 4) {
				/* code 1: unifi_addr */
				envp[j++] = xasprintf("unifi_addr=%d.%d.%d.%d",temp[2],temp[3],temp[4],temp[5]);
			}
		}
 next:
