...
#define UDELAY_COUNT 4000
...
#ifdef  STA_WPS
            /* Check for first click push button Release */

            if (ar7240_gpio_in_val(JUMPSTART_GPIO) && (!push_count)) {
                push_count++;	
                delay = delay - 2000;
                udelay(1000);
            }

            /* Check for second click push button Press */

            if (!ar7240_gpio_in_val(JUMPSTART_GPIO) && (push_count == 1)){
                push_count++;
                udelay(1000);
            }

            /* Check for second click push button Release */

            if(ar7240_gpio_in_val(JUMPSTART_GPIO) && (push_count == 2)) {
               push_count++;
               break;	
            }
#else
        ...
        if (!delay && !push_count) {
#else
	cpl = AP_PUSH_BUTTON; /*For AP PUSH Button */ 
	if (!delay) {
#endif
        ...
        if(push_count < 3) {
            cpl = AP_PUSH_BUTTON ;
            printk("\nar7240: AP push Button pressed...\n" );
        } else if(push_count == 3){
            cpl = STA_PUSH_BUTTON ;
            printk("\nar7240: Client push Button pressed...\n" );
        }
#endif
