...
static int 
ehci_hcd_ar7100_drv_probe(struct device *dev)
{
...
static int 
ehci_hcd_ar7100_drv_remove(struct device *dev)
{
    struct platform_device  *pdev   = to_platform_device(dev);
    struct usb_hcd          *hcd    = dev_get_drvdata(dev);
...
static struct device_driver ehci_hcd_ar7100_driver = {
    .name       =   "ar7100-ehci",
...
static int __devinit ehci_hcd_ar7100_init (void)
{
	printk (DRIVER_INFO " (AR7100_EHCI)");

	return driver_register(&ehci_hcd_ar7100_driver);
}

static void __exit ar7100_ehci_cleanup (void)
{
	driver_unregister(&ehci_hcd_ar7100_driver);
}

device_initcall(ehci_hcd_ar7100_init);
