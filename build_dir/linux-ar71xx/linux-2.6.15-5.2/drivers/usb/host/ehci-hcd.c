...
static void tdi_reset (struct ehci_hcd *ehci)
{
	...
	tmp = readl (reg_ptr);
	tmp |= 0x03; /* embWise added '0x10' need for chipIdea */
	writel (tmp, reg_ptr);
}
...
static int ehci_run (struct usb_hcd *hcd)
{
	 ...
	 */
	hcc_params = readl(&ehci->caps->hcc_params);
    printk("hcc_params addr %#x val %#x hcs_params addr %#x val %#x\n", 
            &ehci->caps->hcc_params, hcc_params, &ehci->caps->hcs_params,
